...
int CantidadPelisActivas()/// funcion para contar cantidad de registros para crear el tamaño del arreglo
{
...
nodoListaPelicula* borrarPeliPorId(nodoListaPelicula*listaPelis, int id)/// Funcion utilizada para borrar una pelicula en particular de la lista
{
...
nodoListaPelicula* borrarLista(nodoListaPelicula* listaPelis)/// Funcion utilizada para borrar la lista entera
{
...
nodoArbolPelicula*modificarPeliEnArbol(nodoArbolPelicula*ArbolPelis,stPelicula auxPeli) ///Funcion invocada en la modificacion de peliculas que cambia la estructura anterior ubicada en el arbol por la de la pelicula modificada
{
...
nodoArbolPelicula*nodoMasDer(nodoArbolPelicula*ArbolPelis)///Funcion utilizada en borrar nodo arbol para encontrar el nodo ubicado mas a la derecha
{
...
nodoArbolPelicula*nodoMasIzq(nodoArbolPelicula*ArbolPelis) ///Funcion utilizada en borrar nodo arbol para encontrar el nodo ubicado mas a la izquierda
{
...
nodoArbolPelicula*borrarNodoArbol(nodoArbolPelicula*ArbolPelis, int idPeli) ///Funcion utilizada para borrar un nodo y eventualmente el arbol entero
{
...
nodoArbolPelicula*borrarArbol(nodoArbolPelicula*ArbolPelis)///Funcion que borrar el arbol entero de izquierda a derecha
{
...
nodoArbolPelicula*altaPelicula(nodoArbolPelicula*ArbolPelis) // Funcion general de alta de pel\EDculas
{
...
//    int cantReg=0;
    ...
    if(archi==NULL) // Chequea si se puede abrir el archivo usando s\F3lo lectura
    {
    ...
    else
    {
        ...
        while(control=='s')
        {
            ...
            peliculaExiste=validarPeliExiste(peliAux.nombrePelicula); // Chequea que el nombre de la pelicula en cuesti\F3n no exista en el archivo para evitar que se duplique mediante un flag
            ...
            do
            {
                printf("Ingrese el nombre del director de la pelicula(Solo caracteres sin espacios): ");
                fflush(stdin);
            ...
            printf("Desea continuar ingresando peliculas(s/n?)"); // Se consulta si se desea continuar ingresando usuarios en esta sesi\F3n
            fflush(stdin);
            scanf("%c", &control);
...
nodoArbolPelicula* bajaPelicula(nodoArbolPelicula*ArbolPelis)//Elimina peliculas pasando usuario.eliminado a valor verdadero
{
    ...
    else
    {
        ...
        {
            fseek(archi, (idPeli)*sizeof(stPelicula), SEEK_SET); // Se lleva el cursor al principio del archivo para moverse desde all\ED
            ...
            fseek(archi, -1*sizeof(stPelicula), SEEK_CUR); // Se lleva el cursor al principio del archivo para moverse desde all\ED
...
nodoArbolPelicula* mostrarPeliParaModif(nodoArbolPelicula*ArbolPelis)//Modificacion de peliculas para administrador
{
    ...
    if(archi!=NULL)
    {
        ...
        if(flag==1) //En caso de que se encuentre la pelicula se muestra por pantalla y se procede a seleccionar el campo a modificar
        {
            ...
            fseek(archi, (id)*sizeof(stPelicula), SEEK_SET); // Se lleva el cursor al principio del archivo para moverse desde all\ED
...
int cargarArregloPeliculasActivas(stPelicula arregloPelis[], int validosPelis)///recorre el archivo para cargar peliculas activas al arreglo que luego se utilizara en varias funciones
{
    FILE *archi;
...
void cargarArregloPeliculas(stPelicula arregloPelis[], int validosPelis)///funcion de carga de arreglo con el total de los registros para la consulta y modificacion por parte del administrador
{
    FILE *archi;
...
///////// **Funciones utilizadas por los listados de orden por titulo y genero **


///Funciones utilizadas para el ordenamiento por genero
...
///Funciones utilizadas en el ordenamiento por titulo
...
///Funcion que imprime los arreglos por pantalla
...
int validarPeliExiste(char nombrePelicula[]) // Valida si el nombre del usuario ingresado actualmente ya existe en el archivo
{
    ...
    archi=fopen(ARCHIPELI, "rb"); // Abre al archivo en modo solo lectura para chequear acceso
    ...
    return flag; // Devuelve el valor de flag para su proceso en la funci\F3n que la invoca
}
...
void mostrarPeliculas(char archiPeli[]) // Funcion auxiliar usada para chequear que el sistema funcione correctamente visualizando los registros ingresados por pantalla -
{
    ...
    else
    {
        ...
        while(!feof(archi)) // Lectura de los datos que existen ene el archivo(Como funci\F3n devuelve la cantidad de datos le\EDdos)
        {
...
void mostrarListaPelis(nodoListaPelicula * lista, char nombreUsuario[]) ///Funcion que muestra las peliculas vistas por cada usuario
{
    ...
    if(aux)
    {
        printf("\n\tEstas son las peliculas vistas por '%s':\n\n", nombreUsuario);
        Sleep(500);
        while(aux)
        {
            printf("\tID Pelicula: %i \n", aux->p.idPelicula);
